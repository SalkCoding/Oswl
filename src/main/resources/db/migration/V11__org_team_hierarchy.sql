-- Organization → Team → Project hierarchy with team-scoped access grants.
--
-- Teams group projects so access can be granted once per team (team_members) instead of
-- once per project (project_members). The two grant paths are combined by OR in
-- ProjectAccessService, so existing project_members rows keep working unchanged.
--
-- Data migration: every existing project is assigned to a "Default" team inside the single
-- "Default Organization" row, keeping pre-migration access identical (team grant or direct
-- project membership). Teams nest at most two levels — enforced by TeamService, not the schema.
--
-- All statements are idempotent (IF NOT EXISTS / WHERE NOT EXISTS) so this migration is safe
-- to (re)apply. db/org_team_hierarchy.sql is the matching ddl-auto=validate reference script.
