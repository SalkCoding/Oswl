...
class SettingsTabAccessTest {
    ...
    @Test
    @DisplayName("systemAdmin → 모든 탭 포함 (admin, security, ai, vcs, cli, cache, webhooks, diagnostics, config-transfer)")
    void accessibleTabsFor_systemAdmin_allTabs() {
        ...
        assertThat(tabs).extracting(SettingsTabAccess.TabSpec::getKey)
                .containsExactlyInAnyOrder("admin", "security", "license-policy", "ai", "vcs", "cli", "cache", "webhooks", "diagnostics", "config-transfer");
    }
