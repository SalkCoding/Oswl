...
class WebPushMigrationTest {
    @Test void migrationsApplyTwiceAndEnforceOwnershipAndDeliveryUniqueness() throws Exception {
        try(var connection=DriverManager.getConnection("jdbc:h2:mem:push-migrations;MODE=PostgreSQL","sa", "")) {
            ...
            for(int repeat=0;repeat<2;repeat++) for(String file:new String[]{"V32__vulnerability_lookup_outcomes.sql","V33__custom_scan_rules.sql","V34__web_push_notifications.sql","V35__database_mutation_locks.sql"}) {
            ...
            try(var statement=connection.createStatement()) {
                try(var result=statement.executeQuery("SELECT COUNT(*) FROM database_mutation_locks")) {result.next();assertThat(result.getInt(1)).isEqualTo(2);}
