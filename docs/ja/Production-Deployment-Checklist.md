...
| 閉域網（エアギャップ）ホスト | 自動ダウンロードには一度だけ外向きのインターネットアクセスが必要です。それがない場合は、開始をクリックする前に `.gguf` ファイル（例: 手動で取得した Gemma — 下記参照）を自分で `embedded-ai/` に配置してください |
| Gemma フォールバック | 自動ダウンロードされることはありません — 標準的な OSS ライセンスではない[Gemma Terms of Use](https://ai.google.dev/gemma/terms)の下にあるため、OsWL はユーザーに代わって再配布しません。低スペック向けフォールバックが必要な場合は自分でダウンロードしてください。[内蔵 AI](Embedded-AI.md)を参照 |
