...
`POST /api/scan` は任意の `idempotencyKey` を受け付けます（英数字・`.`・`_`・`:`・`-` の1〜128文字）。キーはプロジェクト単位でスキャンとともに保持します。同じキーと意味入力では元のscanIdと現在の状態を返し、失敗したスキャンも再実行しません。同じキーで入力が異なる場合はHTTP 409です。新しい解析には新しいキーを使用し、省略した場合は常に新しいスキャンを作成します。認証とプロジェクト権限は毎回確認します。

バージョン付きSHA-256入力ダイジェストはプロジェクトバージョン、提出者メール、コンポーネント識別情報・scope・manifestライセンス・依存経路を含みます。パスワード・rawJson・JSON空白・プロパティ順は除外します。配列順、大文字小文字、nullと空値は保守的に区別し、キーも大文字小文字を区別します。既存スキャンのキーとダイジェストはnullのままです。CLIはキー生成と明示的な再送をサポートします（[CLI連携](CLI-Integration.md)）。インポート、ツール・設定revision固定、共有Library/CVE判定の不変保存は別の残作業です。
