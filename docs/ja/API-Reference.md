...
バージョン付きSHA-256入力ダイジェストはプロジェクトバージョン、提出者メール、コンポーネント識別情報・scope・manifestライセンス・依存経路を含みます。パスワード・rawJson・JSON空白・プロパティ順は除外します。配列順、大文字小文字、nullと空値は保守的に区別し、キーも大文字小文字を区別します。既存スキャンのキーとダイジェストはnullのままです。CLI・インポートの自動再送キー、ツール・設定revision固定、共有Library/CVE判定の不変保存はまだ含みません。
