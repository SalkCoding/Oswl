...
オフライン OSV の照会バッチは、検出内容・未確認キー・ソース鮮度を一つの独立した SERIALIZABLE トランザクションで読み取ります。同時インポートで旧世代の空の結果に新しいカバレッジや日付を付けることを防ぎ、次の照会ではコミット済みの更新を取得できます。既存トランザクション内から呼ぶ場合、この短い照会には別の DB 接続が必要なため、同時読み取り用のプール余裕が必要です。この境界は一つの OSV バッチに限られ、スキャン全体・他のソース・後続の候補有効期限照会・永続的なデータ世代固定は含みません。
