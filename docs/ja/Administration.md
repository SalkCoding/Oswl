...
判定のない既存スキャンは従来のライブ・アーカイブ参照を継続し、過去の判定を再構成しません。確認・猶予状態とレポート生成時刻は現在の値です。Security Center詳細、ゲートなどLibrary/CVEの直接参照は移行が必要で、完全な不変評価履歴ではありません。独立したAI出力はデータ解析判定に含みません。プロバイダー結果の同時保存や部分解析失敗の既存の限界も残り、この保存だけで全プロバイダーの照会成功を証明しません。元のオフライン世代参照はスキャンに残します。保存期間と別の再評価revisionは残作業です。
