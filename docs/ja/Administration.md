...
複数のCPE候補またはオフライン結果リストが同じNVD IDに異なる根拠を返す場合、提供元アダプターは未採点の確認候補1件と代替根拠一覧（nvdApplicability.conflictingRecords）を保持し、照会を未完了にします。JSONオブジェクトのキー順や照合信頼度の違いだけでは競合にしません。直列化した競合も後の読み取りで未確認を維持します。到着順や更新時刻で競合を解消せず、既存共有行やHTTPページ内のすべての重複を調整するものでもありません。
