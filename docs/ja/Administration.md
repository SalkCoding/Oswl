...
CLI 検証と差分基準の読み込みでは、宣言された `github-attributed` プロファイルと内容を照合します。OSV・unresolved ファイルのみを許可し、各情報に ID と出典が一致する原文が必要です。完全バンドルでは情報のある各コンポーネントの未確認状態を保持し、差分を含め未確認状態の削除レコードを拒否します。これはプロファイルの整合性確認であり、出典の真正性や法的承認の証明ではありません。
