...
アプリケーションは OSV 共通修正の判定・理由・対象アドバイザリの ID/revision を `libraries.osv_fix_assessment` に照会試行時刻とともに保持します。インストール済みバージョンに影響しなくても、アップグレードを制限する提供文書を含みます。OSV 照会またはスナップショットが不完全な場合は修正バージョンを保留し、新しい照会では以前の判定を無効にします。これは OSV の範囲の根拠であり、全ソースで検証済みのアップグレード先ではありません。保存済みの判定による画面/PR 候補の選択と原文の自動エクスポートは未完了です。
