...
現在のGitHub Advisoryクライアントが対象エコシステムを照会できる場合、キャッシュのGitHub結果が欠落、NOT_CONFIGUREDまたはUNSUPPORTEDなら、永続キャッシュでも次の解析で再照会します。既存のRESOLVED結果は通常のキャッシュ規則を維持します。完了した照会後のトークン権限やAPIサーバーの変更までは検出せず、出典設定のrevision管理は別の課題です。過去の保存済み判定は変更しません。

保存済み判定にはライブラリIDごとの項目を一つだけ含め、同じ内容でも重複を拒否します。ゲートが一方を選択しレポートが両方を集計する不一致を防ぐため、共通の読み取りを失敗させます。同じライブラリの重複コンポーネントは対応し、判定保存時に一意なライブラリを一度だけ記録します。曖昧な既存記録は自動修正しません。

保存済みスキャン判定はJSONオブジェクトの重複フィールドと末尾の追加JSON値を拒否します。曖昧な保存根拠は読み取りに失敗し、ゲート・レポート・集計・アーカイブエクスポートは現在の共有判定で代用しません。保存済み記録は変更しません。これはJSON構造の検査であり、出典の真正性や内容の正確性を保証するものではありません。文字列として含まれる出典原文には別の検証規則が適用されます。 フィールドの型も検査します。小数の形式バージョン・ライブラリID、数値の列挙値、文字列・数値・真偽値間の暗黙変換を拒否します。正常なcaptureで保存した判定の形式は維持します。

照会完了の検証はOSV・DEPS_DEV・GITHUB_ADVISORY・NVDの出典と、正確なRESOLVED・UNSUPPORTED・NOT_CONFIGUREDの状態を認識します。RESOLVEDが一つ以上必要で、UNAVAILABLE、不明な出典・状態、SNAPSHOT未完了マーカーがある場合は完了と認めません。元の値は診断用に保持します。現在のキャッシュと保存判定は同じ検証を使いますが、適用可能な全出典を照会した証明ではありません。

新しい解析はDEPS_DEVの結果を別途記録します。対応エコシステムのバージョン照会が完了し、要求した全アドバイザリが存在して現在有効であることが必要です。OSVが成功してもdeps.devのバージョン・アドバイザリの根拠が欠落または古い場合はUNAVAILABLEを保持し、照会完了と共通修正バージョンの推奨を保留します。対応エコシステムの旧キャッシュにこの出典結果がなければ、永続キャッシュ設定でも次の解析で再照会します。非対応エコシステムはUNSUPPORTEDです。以前の保存判定は書き換えず、全出典のrevision検証や重複アドバイザリの競合解決を意味するものではありません。
