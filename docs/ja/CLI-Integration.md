...
照会完了の判定は未来のfetchedAt・vulnerabilityLookupAtを拒否し、出典別結果が保存されていれば照会時刻を必須にします。保存判定はキャプチャ時のlookupTimesVerifiedを保持し、後の時計や共有キャッシュで再検証しません。旧保存判定にフラグがない場合は根拠未完了として扱うため、遡って値を補わず新しい解析を実行してください。これはローカル照会時刻の検証であり、提供元データの鮮度を証明しません。出典別結果のない旧liveキャッシュは未来日時の拒否を除いて従来の互換動作を維持します。
