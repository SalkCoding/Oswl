...
保存済みスキャン判定はJSONオブジェクトの重複フィールドと末尾の追加JSON値を拒否します。曖昧な保存根拠は読み取りに失敗し、ゲート・レポート・集計・アーカイブエクスポートは現在の共有判定で代用しません。保存済み記録は変更しません。これはJSON構造の検査であり、出典の真正性や内容の正確性を保証するものではありません。文字列として含まれる出典原文には別の検証規則が適用されます。
