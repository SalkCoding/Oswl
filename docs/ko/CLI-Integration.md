...
현재 GitHub Advisory 클라이언트가 해당 생태계를 조회할 수 있으면, 캐시에 GitHub 결과가 없거나 NOT_CONFIGURED·UNSUPPORTED이면 영구 캐시에서도 다음 분석 시 재조회합니다. 기존 RESOLVED 결과는 정상 캐시 규칙을 유지합니다. 완료 조회 이후 토큰 권한이나 API 서버만 바뀐 경우까지 탐지하는 것은 아니며 공급자 설정 revision 관리는 별도 과제입니다. 과거 보존 판정은 수정하지 않습니다.

보존 판정에는 라이브러리 ID별 항목이 하나만 있어야 하며 내용이 같아도 중복을 거절합니다. 게이트가 한 값을 선택하고 보고서가 둘 다 집계하는 불일치를 막기 위해 공통 읽기에 실패합니다. 같은 라이브러리의 중복 컴포넌트는 지원하며, 판정 저장 시 고유 라이브러리를 한 번만 기록합니다. 모호한 기존 기록을 자동으로 수정하지 않습니다.

보존 스캔 판정은 JSON 객체의 중복 필드와 뒤에 붙은 추가 JSON 값을 거절합니다. 모호한 저장 근거는 읽기에 실패하며 게이트·보고서·요약·보관 내보내기가 현재 공유 판정으로 대체하지 않습니다. 저장된 기록은 수정하지 않습니다. 이는 JSON 구조 검사이며 공급자 진위나 내용의 정확성을 보증하지 않습니다. 문자열로 포함된 출처 원문은 별도의 검증 규칙을 유지합니다. 필드 타입도 검사합니다. 소수인 형식 버전·라이브러리 ID, 숫자 열거형 값, 문자열·숫자·불리언 사이의 암묵적 변환은 거절합니다. 정상 capture로 저장한 판정의 형식은 유지합니다.

조회 완료 검증은 OSV·DEPS_DEV·GITHUB_ADVISORY·NVD 출처와 정확한 RESOLVED·UNSUPPORTED·NOT_CONFIGURED 상태를 인식합니다. RESOLVED가 하나 이상 필요하며 UNAVAILABLE, 알 수 없는 출처·상태와 SNAPSHOT 미완료 표식이 있으면 완료로 보지 않습니다. 원래 값은 진단을 위해 보존합니다. 현재 캐시와 보존 판정이 같은 검증을 사용하지만, 모든 적용 가능한 출처를 조회했다는 증명은 아닙니다.

새 분석은 DEPS_DEV 결과를 별도로 기록합니다. 지원 생태계의 버전 조회가 완료되고 요청한 모든 공지가 존재하며 현재 유효해야 완료입니다. OSV가 성공해도 deps.dev의 버전·공지 근거가 누락되거나 오래됐으면 UNAVAILABLE로 유지해 전체 완료 판정과 공통 패치 추천을 보류합니다. 지원 생태계의 이전 캐시에 이 출처 결과가 없으면 영구 캐시 설정에서도 다음 분석 시 재조회합니다. deps.dev 미지원 생태계는 UNSUPPORTED로 구분합니다. 이전 보존 판정은 재작성하지 않으며, 모든 출처 revision 검증이나 중복 공지 충돌 해결을 의미하지 않습니다.
