...
보존 스캔 판정은 JSON 객체의 중복 필드와 뒤에 붙은 추가 JSON 값을 거절합니다. 모호한 저장 근거는 읽기에 실패하며 게이트·보고서·요약·보관 내보내기가 현재 공유 판정으로 대체하지 않습니다. 저장된 기록은 수정하지 않습니다. 이는 JSON 구조 검사이며 공급자 진위나 내용의 정확성을 보증하지 않습니다. 문자열로 포함된 출처 원문은 별도의 검증 규칙을 유지합니다.
