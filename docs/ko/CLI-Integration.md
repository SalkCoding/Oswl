...
요청·정책·서버 기본값을 합친 최종 EPSS 임계값은 유한한 1 이하 값인지 검증합니다. [0,1]은 규칙을 켜고 유한한 음수는 기존처럼 규칙을 끕니다. NaN·무한대·1 초과 값은 게이트 결과 대신 잘못된 요청 오류를 반환합니다. 조직 최소 정책을 요청이 약화하지 못하도록 하는 조합 규칙까지 구현된 것은 아닙니다.
