...
두 인사이트 모두 프로젝트의 [배포 프로필](Glossary.md)을 고려해 생성됩니다 — 같은 라이선스라도 사내 전용 도구, 네트워크 서비스, 고객에게 배포되는 소프트웨어에서 발생시키는 의무가 서로 다르기 때문에, 요약은 라이선스 계열에 대한 일반적인 설명이 아니라 해당 맥락에서 실제로 발생하는 의무를 명시합니다.
...
**`LICENSE_EXPORT`** 권한(또는 시스템 관리자)이 있는 사용자는 다음을 다운로드할 수 있습니다:

* **NOTICE 파일** — 배포용 저작권 표시 텍스트
* **SPDX SBOM** — 기계 판독 가능한 소프트웨어 명세서

내보내기에는 권한 외에 **프로젝트 멤버십**도 필요합니다. [권한 레이어](Authorization-Layers.md) 참고.
