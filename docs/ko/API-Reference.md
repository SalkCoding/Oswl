...
`POST /api/scan`은 선택적 `idempotencyKey`를 받습니다(영문·숫자·`.`·`_`·`:`·`-` 1~128자). 키는 프로젝트별로 구분하고 스캔과 함께 보존합니다. 같은 키와 의미 입력이면 기존 scanId와 현재 상태를 반환하며, 실패한 스캔도 다시 실행하지 않습니다. 같은 키에 다른 입력을 보내면 HTTP 409입니다. 새 분석은 새 키를 사용하며, 키를 생략하면 항상 새 스캔을 생성합니다. 인증과 프로젝트 접근 권한은 요청마다 확인합니다.

버전이 지정된 SHA-256 입력 해시는 프로젝트 버전, 제출자 이메일, 컴포넌트 식별자·scope·manifest 라이선스·의존 경로를 포함합니다. 비밀번호·rawJson·JSON 공백·속성 순서는 제외합니다. 배열 순서, 대소문자, null과 빈 값은 보수적으로 구별하며 키도 대소문자를 구별합니다. 기존 스캔의 키와 해시는 null로 남깁니다. CLI는 키 생성과 명시적 재전송을 지원합니다([CLI 연동](CLI-Integration.md)). 가져오기, 도구·설정 revision 고정, 공유 Library/CVE 판정의 불변 보존은 별도 남은 작업입니다.
