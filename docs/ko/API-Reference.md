...
버전이 지정된 SHA-256 입력 해시는 프로젝트 버전, 제출자 이메일, 컴포넌트 식별자·scope·manifest 라이선스·의존 경로를 포함합니다. 비밀번호·rawJson·JSON 공백·속성 순서는 제외합니다. 배열 순서, 대소문자, null과 빈 값은 보수적으로 구별하며 키도 대소문자를 구별합니다. 기존 스캔의 키와 해시는 null로 남깁니다. CLI·가져오기 클라이언트의 자동 재전송 키, 도구·설정 revision 고정, 공유 Library/CVE 판정의 불변 보존은 아직 포함하지 않습니다.
