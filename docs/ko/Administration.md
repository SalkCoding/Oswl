...
OSV 공지 ID가 일치하는 기존 발견에서도 수용된 모니터링 결과의 심각도·출처를 병합하고 누락된 CVSS 쌍·요약·CWE를 채우며 수정 버전을 갱신합니다. 오래된 revision은 이러한 변경 전에 제외합니다. 심각도는 여전히 최댓값·충돌 표시의 보수적 규칙을 따르므로 하향 정정의 출처별 재계산이 완료된 것은 아닙니다. 기존 CVSS 쌍을 함께 유지하고 누락된 심각도를 기본값으로 추측하지 않습니다. CVE 별칭만 일치하는 경우의 조정은 별도 잔여 과제입니다.
