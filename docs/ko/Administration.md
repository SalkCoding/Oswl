...
오프라인 OSV 조회 묶음은 취약점 본문·미확인 키·출처 신선도·후보 만료일을 하나의 독립적인 SERIALIZABLE 트랜잭션에서 읽습니다. 동시에 번들을 반입해도 이전의 빈 결과에 새 번들의 정상 상태나 날짜를 붙이지 않습니다. 다음 조회는 커밋된 갱신을 읽을 수 있습니다. 기존 트랜잭션 안에서 호출하면 짧은 조회 동안 별도 DB 연결이 필요하므로 동시 조회를 위한 풀 여유를 확보해야 합니다. 이 경계는 OSV 조회 묶음 하나에 한정되며 전체 스캔·다른 출처·영속적인 데이터 세대 고정까지 포함하지 않습니다.
