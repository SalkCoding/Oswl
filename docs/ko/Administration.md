...
스캔 수집의 OSV 정규화 결과도 공유 행 갱신 전에 기록합니다. 실제 매핑된 조회 식별자, 발견과 수정 버전 충돌, 공통 수정 후보와 선택 사유, 공지 revision/digest 및 제공된 만료 시각을 보존합니다. 정상 빈 결과와 실패·누락 결과를 구분하며, 불완전한 오프라인 스냅샷은 원래 결과를 유지하면서 관측 완전성을 낮춥니다. 미지원 조회의 관측은 만들지 않습니다. OSV 공지 전체 원문이나 철회 이력을 추가하는 것은 아니며, 기록된 수정 후보를 새로 검증한 패치 권고로 간주하지 않습니다.
