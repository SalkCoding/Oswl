...
모니터링은 OSV 공지 ID가 정확히 일치하는 발견을 우선하고, 없으면 같은 CVE 별칭의 기존 발견에 연결합니다. 두 경로 모두 공지별 revision 검사 후 심각도·출처를 병합하고 누락된 CVSS 쌍·요약·CWE를 채우며 중복 발견 알림을 만들지 않습니다. 저장된 수정 버전 교체는 같은 공지 ID에만 허용합니다. 기존 CVSS 쌍을 함께 유지하고 누락된 심각도를 추측하지 않습니다. 기존 이력·혼합 공급자·불완전 기여 이력은 보수적 심각도 병합을 유지하며 전체 별칭 기여와 철회 조정은 아직 미완료입니다.
