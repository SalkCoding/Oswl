...
KEV 벌크 수집은 유효하고 미래가 아닌 `dateReleased` 시각을 요구하고 UTC 날짜를 보존합니다. 누락·손상·날짜만 있는 입력은 빌드 날짜로 대체하지 않고 수집 실패로 처리합니다. 2026-09-13 확인한 [CISA 공식 스키마](https://raw.githubusercontent.com/cisagov/kev-data/main/known_exploited_vulnerabilities_schema.json)를 따릅니다. 릴리스 시각 검증이며 전체 카탈로그 스키마나 재배포 권한 검증은 아닙니다.

KEV 벌크 수집은 선언된 정수 건수를 취약점 배열 길이와 대조하고, 비공백 카탈로그 버전을 요구하며, 잘못되거나 중복된 CVE 식별자를 거절합니다. 잘못된 행을 버려 부분 성공 번들을 만들지 않습니다. 온라인 클라이언트의 기존 목록 검증과 맞춘 것이며, 설명 필드 전체 스키마와 릴리스 간 롤백·충돌 정책은 별도 범위입니다.
