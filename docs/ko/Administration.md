...
애플리케이션은 OSV 공통 수정 판정·사유·참여 공지 ID/revision을 `libraries.osv_fix_assessment`에 조회 시도 시각과 함께 보존합니다. 현재 설치 버전에는 영향이 없어도 업그레이드를 제한하는 제공 공지도 포함합니다. OSV 조회나 스냅샷이 불완전하면 수정 버전을 보류하고 새 조회는 이전 판정을 무효화합니다. 이는 OSV 범위의 근거이며 모든 출처를 검증한 업그레이드 대상이 아닙니다. 저장된 판정을 화면/PR 후보 선택에 연결하는 작업과 원문 자동 내보내기는 아직 남아 있습니다.
