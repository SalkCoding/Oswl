...
새 NVD 조회는 공유 행 조정 전에 관측 기록도 저장합니다. 패키지 식별자, UTC 관측 시각, 공급자 조회 완전성, 활성·철회 관측과 제공된 revision/configuration 근거를 담습니다. 실패하거나 누락된 오프라인 조회는 미완료로 남기고, 미지원 출처의 관측을 만들지는 않습니다. 근거 저장에 실패하면 수집을 실패 처리합니다. 기록은 공유 CVE/라이브러리 삭제와 스캔 보관 후에도 남지만 소유 스캔 삭제 시 함께 삭제됩니다. V47 마이그레이션을 적용하고 DB 백업에 테이블을 포함해야 합니다. 기존 스캔은 역으로 채우지 않습니다. 다른 공급자·자동 재계산·스캔/오프라인 출력의 기록 왕복은 아직 미구현이며, 재배포 권한이나 변조 방지 보증을 추가하지 않습니다.
