...
`osv.jsonl`의 취약점 레코드에는 원래 OSV 문서인 `osvAdvisory` JSON 객체를 선택적으로 넣을 수 있습니다. `id`는 `osvId`와 일치하고, `modified`는 유효한 시각이며 `affected`는 배열이어야 합니다. 저장소는 credits·references를 포함한 제공 문서를 보존하고 오프라인 조회는 식별자·설명·CWE·심각도·CVSS를 원문에서 읽고 영향 여부와 수정 버전을 다시 판정합니다. 공통 수정 판정에는 모든 관련 레코드의 원문과 최신·완료된 출처 조회 상태가 필요합니다. 원문 없는 기존 레코드는 종전 조회 동작을 유지하지만 공통 수정의 근거가 되지 않습니다. 이 필드는 원천 자료의 수집·재배포 허가를 부여하지 않습니다. 자동 수집기와 스캔 기반 재내보내기는 아직 이 원문을 전달하지 않으므로 원본 번들을 보관하고 권한·고지를 별도로 확인해야 합니다.

애플리케이션은 OSV 공통 수정 판정·사유·공지 revision·현재 영향받는 ID/별칭·배타적 만료 시각을 `libraries.osv_fix_assessment`에 조회 시도 시각과 함께 보존합니다. 설치 버전에 영향이 없어도 업그레이드를 제한하는 공지는 revision 근거에 포함합니다. 화면과 PR 후보 선택은 조회가 완료되고 저장된 모든 취약점의 근거가 있으며 수정 충돌이 없는 유효한 공통 후보만 사용합니다. 개별 공지의 수정 정보는 별도로 유지합니다. 온라인 후보 표시는 7일 후 만료되며 오프라인은 원래 OSV 기준일과 설정된 일 단위 노후화 기간을 따르므로 재조회만으로 오래된 데이터의 수명이 늘어나지 않습니다. 영향 ID나 만료 시각이 없는 기존 판정은 갱신 전까지 미확인입니다. 장기·영구 캐시는 지운 뒤 다시 스캔해야 새 근거를 얻을 수 있습니다. PR은 생성 직전에 대상을 다시 검증합니다. 원문 자동 수집·내보내기는 아직 남아 있습니다.
