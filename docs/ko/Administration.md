...
GHSA 벌크 수집 보조 클래스는 정상 0건 조회를 명시적인 컴포넌트 키로 보존하고, 조회 불가·실패는 `unresolvedKeys`로 구분합니다. 부분 조회 예외의 확보된 취약점과 수정 버전 보류 상태를 유지합니다. 미지원·미설정 클라이언트에는 조회를 보내지 않습니다. 이 클래스는 아직 CLI writer에 연결되지 않았으므로 `--sources github-advisory`를 활성화하거나 번들 출처 추적을 완료한 변경은 아닙니다.
