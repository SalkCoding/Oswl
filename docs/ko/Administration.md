...
컴포넌트 상세도 보존된 취약점·라이선스·조회 근거를 표시합니다. 잘못된 판정이나 라이브러리 식별자 누락은 최신 취약점으로 대체하지 않고 실패합니다. 미확정 CPE 발견에는 개별 검토 표시를 붙입니다. 현재 업데이트 권고와 재검증을 거치는 PR 조치는 과거 발견과 구분해 유지합니다. 현재 발견 근거와 보존 근거가 정확히 일치할 때만 AI 설명을 연결하며 과거 식별자로 현재 DB 행을 추정하지 않습니다. 검토·예외 처리, 패키지 메타데이터와 환경 점수의 환경 설정은 현재 값을 사용하며 화면 안내로 구분합니다.

보안 센터 CSV는 선택한 스캔의 보존된 판정에서 구성 목록, 심각도별 건수, CPE 검토 후보, 패치 가능 여부, 라이선스와 조회 근거를 읽습니다. 이후 공유 라이브러리 갱신으로 이 값들을 바꾸지 않습니다. 보관 처리로 스캔 컴포넌트가 삭제돼도 보존된 행을 내보내며, 컴포넌트가 없으면 범위와 검토·예외 처리 열은 빈칸으로 남깁니다. 컴포넌트가 있으면 현재 검토 상태를 사용합니다. 판정이 없는 과거 스캔은 기존 라이브 데이터 방식을 유지하고, 잘못된 판정은 최신 데이터로 대체하지 않고 실패합니다. 대화형 목록과 인쇄 행도 취약점·라이선스 필터와 위험도 정렬에 보존 근거를 사용하며, 현재 검토 상태와 릴리스 정보는 구분해 유지합니다.
...
온라인·오프라인 NVD 결과는 제공된 `lastModified`가 잘못된 형식이거나 미래이면 활성 공지나 상태 없는 공지도 미확인으로 처리합니다. 발견과 원래 근거는 보존하되 조회 완료로 표시하지 않습니다. 수정 시각 필드가 없는 과거 활성 레코드는 계속 읽을 수 있으며, 철회 공지를 제거하려면 여전히 유효한 수정 시각이 필요합니다.

`vulns[].nvdApplicability`를 제공할 때는 보존한 NVD 근거를 담은 공백이 아닌 문자열이어야 합니다. 문자열이 아닌 값이나 공백은 근거 없음으로 조용히 바꾸지 않고 반입 전체를 거절합니다. 필드 누락/null은 과거 번들과 호환되지만 환경 적용 조건을 평가했다는 증거는 아닙니다. 스캔 내보내기는 configuration 근거를 보존하되 원천 기준일을 갱신하지 않습니다. 라이브러리 조회는 출처 목록을 별도로 읽어 여러 출처가 있는 CVE 하나가 발견 목록에 반복되지 않도록 합니다.

EPSS 점수가 있는 스캔 내보내기와 EPSS를 선택한 CLI 빌드는 FIRST/Empirical Security 출처 및 [데이터 안내](https://www.first.org/epss/data), [FAQ](https://www.first.org/epss/faq), [FIRST 서비스 약관](https://www.first.org/about/policies/terms) 링크를 포함합니다. 2026-09-21 확인: FAQ는 출처 표시를 요청하고, 약관은 고지 보존을 조건으로 사이버보안 목적의 제한적 복제·배포를 설명합니다. 이를 정규화한 고객 DB나 외부 호스팅 CSV의 포괄 재배포 승인으로 적용하지 않습니다. manifest는 `rightsStatus: unreviewed`를 유지하며 SPDX 라이선스를 부여하지 않습니다. `github-attributed`는 EPSS 레코드를 제외하되 전달받은 고지는 보존합니다. API는 개별/소량 조회용이며 대량 동기화에는 일별 CSV를 사용합니다.

앱 내보내기는 다운로드를 반환하기 전에 완성된 ZIP의 압축 해제 후 파일/번들 크기, 메타데이터 크기, 압축률과 JSONL 행 길이를 반입 제한으로 검사합니다. 반복이 많아 압축률 제한을 넘는 항목은 비압축으로 저장해 근거 전체를 보존하므로 다운로드 크기가 커질 수 있습니다. 다른 제한 초과 시 공지 근거를 자르지 않고 내보내기를 실패 처리합니다. 검증에 임시 디스크 공간을 사용하고 이후 staging 파일을 정리합니다. 이 검사가 원천의 완전성이나 재배포 권한을 입증하지는 않습니다.
...
여러 CPE 후보 또는 오프라인 결과 목록이 같은 NVD ID에 서로 다른 근거를 제공하면, 모든 관측의 수명 근거와 `lastModified`가 유효하고 최신 관측이 하나로 정해질 때만 이를 선택합니다. 최신 시각의 내용 충돌, 수정 시각 누락 또는 잘못된 근거가 있으면 미채점 검토 후보와 대안 근거 목록(nvdApplicability.conflictingRecords)을 보존하고 조회를 미완료로 표시합니다. JSON 객체 키 순서나 매칭 신뢰도 차이만으로는 충돌로 처리하지 않습니다. 직렬화한 충돌도 이후 조회에서 미확인으로 유지합니다. 선택은 도착 순서에 의존하지 않으며 불완전한 출처 조회를 완료로 바꾸거나 기존 공유 행까지 조정하지는 않습니다.

수집 결과 반영 시 기존 NVD 관측에 유효한 revision이 있으면 오래되거나 순서를 확인할 수 없는 결과, 같은 revision에서 보존 내용이 다른 결과로 덮어쓰지 않습니다. 기존 근거를 유지하고 NVD 조회를 미완료로 표시합니다. JSON 키 순서와 동일 시각의 표기 차이는 허용합니다. 이 방어는 과거 revision 누락의 복원, 전체 관측 이력 저장, 다중 출처 필드 재계산 또는 저장 발견의 철회 처리를 구현한 것은 아닙니다.

새 NVD 조회는 공유 행 조정 전에 관측 기록도 저장합니다. 패키지 식별자, UTC 관측 시각, 공급자 조회 완전성, 활성·철회 관측과 제공된 revision/configuration 근거를 담습니다. 실패하거나 누락된 오프라인 조회는 미완료로 남기고, 미지원 출처의 관측을 만들지는 않습니다. 근거 저장에 실패하면 수집을 실패 처리합니다. 기록은 공유 CVE/라이브러리 삭제와 스캔 보관 후에도 남지만 소유 스캔 삭제 시 함께 삭제됩니다. V47 마이그레이션을 적용하고 DB 백업에 테이블을 포함해야 합니다. 기존 스캔은 역으로 채우지 않습니다. 자동 재계산·스캔/오프라인 출력의 기록 왕복은 아직 미구현이며, 재배포 권한이나 변조 방지 보증을 추가하지 않습니다.

지속 모니터링도 각 라이브러리의 CVE를 변경하기 전에 OSV 결과를 기록합니다. 빈 결과·실패·누락도 포함하며, 공유 라이브러리는 한 번 조회하고 참여한 최신 스캔마다 한 번씩 연결하므로 중복 컴포넌트로 기록이 늘어나지 않습니다. OSV 기록의 observationKind는 SCAN_ENRICHMENT와 CONTINUOUS_MONITORING을 구분하며, 이 필드가 없는 이전 기록은 스캔 수집에서 생성되었습니다. 이 연결은 원래 스캔 판정을 다시 쓰거나 나중에 발견한 취약점을 당시 알고 있었다고 표시하지 않습니다. 전체 배치의 기록을 저장한 후 CVE 변경과 알림을 시작하므로 기록 저장 실패 시 해당 주기는 이러한 부수 효과 없이 중단됩니다. 먼저 저장된 관측 기록은 남습니다. 모니터링은 현재 OSV만 기록합니다.

모니터링도 스캔 수집과 같은 기존 MAL 공지 규칙을 적용합니다. 수용된 신규 또는 같은 공지 ID의 기존 MAL 발견은 CRITICAL로 반영하고 공유 라이브러리를 악성으로 표시합니다. 악성 표시는 CVE·알림 행과 함께 커밋하므로 알림 저장 실패 시 함께 롤백합니다. 거절된 오래된 revision은 표시를 변경하지 않습니다. 기존 악성 표시의 자동 해제나 철회를 구현한 것은 아닙니다.

OSV revision 필터·스캔 수집·모니터링은 같은 선택 기준을 사용하며 행 순서와 무관하게 CVE 별칭보다 정확한 공지 ID를 우선합니다. 정확한 공지 행이 중복되거나 정확한 일치 없이 CVE 별칭 행이 여러 개이면 모호한 매칭으로 처리합니다. 원래 관측은 보존하고 조회를 미완료로 표시하며 어느 행도 임의 갱신하지 않습니다. 저장 단계에서 처음 발견된 모호함은 저장 실패로 처리합니다. 모니터링은 OSV 공지 ID가 정확히 일치하는 발견을 우선하고, 없으면 같은 CVE 별칭의 기존 발견에 연결합니다. 두 경로 모두 공지별 revision 검사 후 심각도·출처를 병합하고 누락된 CVSS 쌍·요약·CWE를 채우며 중복 발견 알림을 만들지 않습니다. 신규 발견 알림과 통지 데이터는 라이브러리의 모든 별칭을 처리한 뒤 최종 저장 심각도·수정 버전으로 만듭니다. 뒤의 별칭이 심각도를 높이거나 수정 후보를 보류했는데 앞선 낮은 심각도·후보가 신규 알림에 남지 않게 합니다. 저장된 수정 버전 교체는 같은 공지 ID에만 허용합니다. 모니터링은 같은 저장 발견에 연결된 수용 공지들의 수정 후보도 모읍니다. 서로 다른 후보는 충돌 근거로 보존하고 발견·신규 알림의 확정 후보를 보류합니다. 거부된 revision의 후보는 합치지 않습니다. 생성부터 완전한 OSV 단독 이력이 있고, 명시적 만료 시각이 유효한 완전 응답에 저장된 모든 공지가 수용 가능한 revision/digest로 포함되며, 모두 반입 충돌 없이 같은 수정 버전을 제공할 때만 기존 충돌을 해제합니다. 공지 누락·수정 버전 미상·불완전 coverage·혼합 출처는 보류하며 과거 알림은 바꾸지 않습니다. OSV 단독 및 OSV/GitHub 합의 경로는 충돌 해제 전에 저장된 OSV 공지 전체에 같은 검사를 적용합니다. 과거 별칭이 다른 CVE로 바뀌면 같은 수정 버전을 제시해도 이전 행의 충돌을 해제하지 않으며, 공지 누락·중복·식별 불일치·과거/검증 불가 revision·만료/만료 미상 응답은 해제를 막습니다. OSV 기여 이력이 없거나 비어 있거나 불완전한 과거 행도 충돌 해제를 막습니다. 현재 일부 공급자의 합의만으로 과거 충돌에 기여한 전체 공지를 확인했다고 간주하지 않습니다. 완전하게 기록된 이력의 현재 합의는 회복할 수 있으며, 누락된 GitHub 기여 이력 자체를 복원하는 것은 아닙니다. 별도 회복 가능 이력 표시로 일시적인 coverage 부족·만료와 과거 근거 자체의 부재를 구분합니다. 이후 완전한 갱신에서 현재 심각도 기여를 복원하고, 모든 기록 공지가 현재 근거 검사를 통과해야 수정 충돌을 해제합니다. 공지 식별이나 revision/digest 근거 누락은 회복 가능 표시도 무효화하며 과거 불완전 이력을 추정하지 않습니다. SQL migration 없이 기존 OSV 근거 JSON에 필드를 추가합니다. 이전 완전 JSON은 읽을 수 있지만 엄격한 JSON 읽기를 사용하는 이전 바이너리는 새 필드를 읽지 못하므로 다운그레이드에는 업그레이드 전 데이터 백업이 필요합니다. deps.dev 보강 경로도 정확한 OSV 공지를 우선하며 신규 발견을 만들 때를 포함해 같은 공지 ID의 수정 버전만 사용합니다. CVE 별칭만으로 공지별 수정 버전을 확정하지 않습니다. 기존 CVSS 쌍을 함께 유지하고 누락된 심각도를 추측하지 않습니다. 기존 이력·혼합 공급자·불완전 기여 이력은 보수적 심각도 병합을 유지하며 전체 별칭 기여와 철회 조정은 아직 미완료입니다.

모니터링은 조회와 관측 기록을 마친 뒤 해당 배치의 공유 CVE 변경과 모든 프로젝트 알림 행을 하나의 트랜잭션으로 저장합니다. 취약점 또는 알림 저장 실패 시 둘 다 롤백하므로 다음 주기에 다시 발견할 수 있습니다. 감사 이벤트와 외부 알림은 커밋 이후에 실행하며 감사 기록 실패는 로그에 남기되 알림 시도를 막지 않습니다. 앞서 커밋된 관측 기록은 유지합니다. 이 변경 전 유실된 알림의 복구, 종료·전송 실패 후 영속적인 전송 재시도, 모든 동시 중복 삽입 방지를 제공하는 것은 아닙니다. 대형 배치의 트랜잭션·잠금 예산은 부하 검증이 필요합니다.

스캔 수집에서는 GitHub의 정규화 조회 결과와 deps.dev 버전 응답도 기록합니다. deps.dev 공지 상세는 해당 컴포넌트가 요청한 ID만 포함합니다. GitHub 조회 상태와 부분 발견을 유지하고, deps.dev의 누락 공지 ID는 별도로 남깁니다. 버전 조회 누락/실패 또는 요청 공지 누락/노후화가 있으면 관측은 미완료이며, 정상 버전 조회에서 공지 키가 없는 경우는 완료된 빈 조회입니다. 최신 릴리스는 버전 메타데이터로 저장하며 수정 버전으로 간주하지 않습니다. 온라인 GitHub 조회는 제공된 updatedAt과 유효한 withdrawnAt을 보존하고 철회 관측을 활성 발견과 별도 목록으로 전달합니다. 후속 페이지 실패나 활성/철회 충돌에서도 양쪽 관측을 기록하며 캐시 발견은 유지합니다. 시각은 관측값이며 검증된 revision 순서나 공유 발견 삭제 허가가 아닙니다. 온라인에서 제공된 updatedAt이 잘못된 타입·날짜 문법·미래 시각이면 독립적으로 매칭한 발견은 유지하되 조회를 미완료로 처리하고 수정 후보를 보류합니다. 오프라인도 같은 현재 시각 검사를 사용합니다. revision 메타데이터 누락/null은 미상으로서 기존 조회 동작을 유지하며 revision 순서의 증거로 간주하지 않습니다. 누락·손상 수명 근거, 전체 과거 이력 복원, 오프라인 번들 전달, 출처 조정은 추가 작업입니다. 기존 관측 JSON에 선택 필드가 추가되어 엄격한 이전 리더로 다운그레이드할 때 업그레이드 전 백업이 필요할 수 있습니다. 필드 의미는 [공식 스키마](https://docs.github.com/en/graphql/reference/security-advisories#securityadvisory)를 따르며 기존 [GitHub Advisory Database 출처 표시 조건](https://docs.github.com/en/site-policy/github-terms/github-terms-for-additional-products-and-features#advisory-database)을 유지합니다. 관측 기록을 새로 내보내기에 포함하지 않습니다. GitHub 일괄 수집 보조 클래스는 부분 조회를 포함해 활성/철회 관측을 SnapshotVuln의 githubUpdatedAt/githubWithdrawnAt 필드로 변환합니다. 명시적인 온라인 전체 CLI 빌드가 이 보조 클래스와 writer를 연결합니다. 실행 조건은 아래 CLI 설명을 참조합니다. 앱 반입은 이를 보존하고 오프라인 조회에서 철회 관측과 활성 발견을 분리합니다. 노후·기준일 누락·상태 충돌은 미완료로 유지하며 미래 철회 시각은 유효한 철회로 확정하지 않고 수정 후보 없는 미확인 발견으로 남깁니다. 필드 없는 기존 번들은 이전 동작을 유지하고 잘못된 시각 타입·문법은 반입을 거절합니다. 없는 선택 필드는 출력하지 않으며 새 번들의 소비자는 해당 필드를 이해해야 합니다. 공유 CVE 행으로 재구성하는 앱 내보내기에는 아직 수명 이력이 보존되지 않습니다.

CLI 패키지 번들은 unresolved.jsonl의 출처별 상태 갱신을 위해 formatVersion 4를 사용합니다. unresolvedSources는 지정 출처의 미확인을 추가하고 resolvedSources는 지정 출처의 표식만 해제합니다. 전체 반입에서도 키별로 적용하여 다른 출처나 대상 목록의 미확인을 지우지 않습니다. 따라서 부분 갱신 후 완전한 갱신으로 복구할 수 있습니다. 소유 출처를 모르는 구형 표식은 미확인으로 유지합니다. 현재 조회는 남은 표식이 하나라도 있으면 보수적으로 미완료로 취급합니다. 버전 4 사용 전 반입기를 업그레이드해야 하며 기존 형식 반입은 유지합니다. 앱 내보내기도 저장된 소유 출처를 formatVersion 4로 보존합니다. 라이브러리의 미분석 상태에서 생긴 추가 불확실성과 소유 출처가 불명인 표식은 legacyUnresolved 근거로 유지하므로, 지정 출처의 정상화만으로 이를 해제하지 않습니다. 출처별 표식이 없는 내보내기는 기존 형식 선택을 유지하며 제한된 출처 표시 프로필은 계속 전체 미확인으로 처리합니다.

동일한 공지 ID의 구형 OSV 요약 기록들이 서로 다른 수정 버전을 제공하면 오프라인 조회는 기록과 충돌 후보를 보존하되 각각의 수정 버전을 보류하고 조회를 미완료로 처리합니다. 입력 순서로 우세 후보를 선택하지 않습니다. 구형 기록의 수정 버전이 일치하면 기존 동작을 유지하며, 일치를 revision 순서의 증거로 간주하지 않습니다. 원문을 가진 공지는 기존 revision 검증을 사용합니다.

OSV 증분에서는 미확인 컴포넌트의 갱신 결과가 없거나 부분적이라는 이유로 이전 발견을 제거하지 않습니다. 결과가 없으면 이전 행을 유지하고, 부분 결과는 서로 다른 이전/새 발견 객체를 함께 보존하며 우세한 revision을 임의 선택하지 않습니다. 미확인 표식이 남으므로 보존된 근거를 완료 조회나 확정된 공통 수정 버전으로 간주하지 않습니다. 완료된 명시적 빈 결과는 이전 발견을 대체할 수 있습니다. 이 보존 규칙은 아직 deps.dev 공지 연결을 다루거나 GitHub 증분을 활성화하지 않습니다.

내부 writer의 별도 호출 경로는 발견·출처 기준일·수집 대상 목록을 명시적으로 받으면 GitHub 전체 데이터를 github-advisory.jsonl로 작성합니다. 수명 필드·정상 빈 조회·미확인 키와 manifest·기존 GitHub 출처 고지를 보존하며 이전 번들 reader도 해당 기록을 유지합니다. 명시적인 온라인 전체 CLI 빌드는 이 경로를 사용합니다. 불완전 갱신의 이전 근거 보존을 구현하기 전까지 GitHub 증분 작성은 거절하며, 기존 github-attributed 프로필은 여전히 출처가 확인된 OSV 원문만 허용합니다.

번들 writer를 직접 호출해도 선택한 미지원 출처는 임시 파일 작성이나 출력 교체 전에 거절합니다. 내부 호출이 CLI 검사를 우회하여 요청한 출처를 조용히 누락한 번들을 성공으로 만드는 것을 방지합니다. 현재 writer 지원 출처는 osv, depsdev, epss, kev이며 수집기 추가 시 writer도 명시적으로 연결해야 합니다.

온라인에서도 GitHub의 미래 철회 시각은 유효한 철회로 확정하지 않습니다. 패키지 식별과 설치 버전의 영향 범위가 별도로 일치하면 발견과 제공된 미래 시각을 유지하되, 조회는 미완료이고 수정 버전은 확정하지 않습니다. 영향 범위 밖 버전은 발견으로 추가하지 않습니다. 문법이 잘못된 철회 시각은 여전히 조회를 미완료로 만들며 해당 노드에서 매칭된 발견을 확정하지 않습니다.

일괄 수집 보조 클래스가 잘못된 updatedAt 문자열을 포함한 GitHub 미완료 조회를 받으면 해당 선택 시각만 알 수 없음으로 변환하고, 매칭된 발견·미확인 컴포넌트 키·수정 후보 보류 상태는 유지합니다. 함께 받은 정상 발견과 뒤따르는 패키지도 계속 수집합니다. 문법상 유효한 미래 시각은 미확인 근거로 보조 클래스 결과에 남깁니다. 잘못된 시각 문자열 자체는 타입이 정해진 번들 필드에 보존하지 않으므로 완전한 원문 보존을 의미하지 않습니다.

스캔 수집의 OSV 정규화 결과도 공유 행 갱신 전에 기록합니다. 실제 매핑된 조회 식별자, 발견과 수정 버전 충돌, 공통 수정 후보와 선택 사유, 공지 revision/digest 및 제공된 만료 시각을 보존합니다. 정상 빈 결과와 실패·누락 결과를 구분하며, 불완전한 오프라인 스냅샷은 원래 결과를 유지하면서 관측 완전성을 낮춥니다. 미지원 조회의 관측은 만들지 않습니다. OSV 공지 전체 원문이나 과거 철회 이력을 복원하는 것은 아니며, 기록된 수정 후보를 새로 검증한 패치 권고로 간주하지 않습니다.

OSV 결과는 철회·영향 범위 필터링 전에 수명 메타데이터도 보존합니다. 공지 ID, 조회와 상세 revision, 공급된 철회 값과 필드 존재 여부, 해석한 철회 상태, revision 일치 여부 및 원문 digest를 담습니다. 철회·미확인·상세 조회 실패도 남기며 상세가 없으면 UNKNOWN 상태로 두고 digest를 만들지 않습니다. 스캔 수집과 모니터링 기록에 같은 정보를 저장합니다. 철회 상태만으로 발견을 삭제하지 않으며 revision 일치, 전체 조회 완전성 및 다른 출처를 함께 조정해야 합니다. 새로 관측한 응답의 메타데이터이며 전체 원문이나 과거 이력 보충이 아닙니다. 필드 의미는 [OSV 공식 스키마](https://ossf.github.io/osv-schema/#withdrawn-field)를 따르며, 스키마가 개별 공급자 데이터의 재배포 권한을 부여하는 것은 아닙니다.

스캔 수집 중 DB 저장 실패(낙관적 잠금 충돌 포함)가 발생하면 판정 캡처 전에 중단하고 독립적인 실패 처리로 스캔을 실패 표시합니다. 일부 발견만 저장된 상태를 완료 판정으로 발행하지 않습니다. 이 실패 처리만으로 동시 쓰기 보호나 공유 라이브러리 전체 갱신의 원자성을 추가한 것은 아니므로 앞서 커밋된 쓰기는 남을 수 있습니다.

V49는 `library_cves.row_version`의 NOT NULL 버전 카운터를 추가합니다. CVE 저장과 라이브러리 cascade merge는 오래된 행 버전을 거절해 최신 발견의 덮어쓰기를 방지합니다. 저장소는 flush 성공 후 순차 출처 조회에서 재사용하는 분리 객체에 버전을 반영하며 일괄 저장의 충돌은 해당 배치를 롤백합니다. 거절된 쓰기는 스캔 실패 경로로 전달하며 오래된 근거로 자동 재시도하지 않습니다. 배포 전 V48 다음에 V49를 적용하고 백업·rollback 시 열을 유지하며 카운터를 갱신하지 않는 이전 writer를 혼용하지 마십시오. 기존 CVE 행의 보호이며, 발견의 동시 중복 생성·라이브러리 전체 메타데이터·스캔/모니터링 전체의 원자성을 보장하지 않습니다.

V48은 `library_cves.osv_revision_evidence` nullable 열에 공지 ID별로 수용한 OSV revision/digest를 저장합니다. 스캔 수집과 지속 모니터링은 오래되거나 순서를 확인할 수 없거나 같은 revision에서 digest가 다른 관측이 기존 발견을 갱신하지 못하게 합니다. 이미 근거가 있는 공지는 유효한 digest도 필요하며, 더 새로운 유효 revision은 반영할 수 있습니다. 원래 수신 응답은 필터 전 관측 기록에 남습니다. 수집은 거절된 OSV 조회를 미완료로 표시하고 공통 수정 후보를 보류하며 모니터링은 거절된 갱신을 건너뜁니다. 저장 revision JSON이 손상되면 방어를 해제하지 않고 실패합니다. 근거 없는 기존 행은 역으로 채우지 않고 다른 공지 ID는 독립적으로 추적합니다. V48 자체는 출처별 필드 재계산·자동 철회·동시성 잠금을 제공하지 않으며, 행 버전 검사는 V49에서 별도로 추가합니다. 배포 전 V48을 적용하고 rollback 시 열을 유지해야 합니다. 이전 애플리케이션은 이 근거를 관리하지 않으므로 서로 다른 버전의 writer를 혼용하지 마세요. 스캔/오프라인 export에는 아직 이 열이 보존되지 않습니다.

OSV 응답이 resolved여도 스냅샷 범위가 미확인이면 심각도 재계산을 허용하지 않습니다. 스캔 수집은 스냅샷 조회 상태를 저장 근거 처리에 전달하고, 모니터링은 원래 컴포넌트 키와 매핑된 OSV 조회 키를 모두 검사합니다. 미확인 키가 있으면 심각도 기여 이력을 불완전으로 기록하고 기존 심각도·CVSS를 낮추지 않습니다. 명시된 유효기간이 지난 OSV 결과도 저장 시 미완료로 처리합니다. 발견과 revision 근거는 유지하지만 개별·공통 수정 후보는 보류하고 심각도를 낮추지 않습니다. 저장만으로 원래 만료 시각을 연장하지 않습니다. 명시적 만료 시각이 없는 결과는 기존 호환 동작을 유지하며, 이 검사만으로 무기준일 근거의 최신성을 입증하는 것은 아닙니다.

수용된 OSV revision 근거에는 이제 공지별 심각도와 CVSS 점수·벡터 쌍도 저장합니다. 생성 시점부터 완전한 이력이 기록된 OSV 단독 행에만 심각도를 재계산합니다. 현재 공지들의 최댓값과 충돌 여부를 사용하고 CVSS 쌍은 같은 심각도의 한 공지에서 가져옵니다(유효 점수 최댓값, 동률이면 공지 ID 순). 명시적으로 심각도가 없는 관측은 미평가 상태이며 발견을 철회·삭제하지 않습니다. 기존 revision JSON·기존 행·혼합 공급자·불완전 조회·revision 없는 기여·이력에 아직 없는 새 CVE 별칭은 하향 정정의 근거로 인정하지 않고 보수적 병합을 유지합니다. 이들의 조정은 잔여 과제입니다. 이미 완전한 OSV 단독 이력에는 완전한 현재 응답의 동일 CVE 별칭 기여를 재계산 전에 함께 기록할 수 있습니다. 모든 별칭의 revision·digest를 수용할 수 있어야 하며 다른 저장 행이나 모호한 행에 속하면 허용하지 않습니다. 따라서 응답 순서와 무관하게 심각도·CVSS를 재계산하지만 과거 불완전 이력을 복원하거나 철회를 적용하는 것은 아닙니다. 확장 JSON은 기존 V48 열을 사용하며 이전 애플리케이션이 읽지 못할 수 있으므로 writer를 혼용하거나 호환 DB 백업 없이 애플리케이션만 rollback하지 마십시오. 스냅샷 export는 아직 이 근거를 운반하지 않습니다.

컴포넌트 스냅샷에 같은 OSV ID의 원문이 여러 개 있으면 모든 원문의 revision·수명 근거가 유효하고 보존된 수정 후보 충돌이 없을 때 최신 revision을 선택합니다. 최신 시각의 내용 충돌, 미래 revision, 원문 없는 과거 형식과의 혼합, 미확인 수명 근거는 계속 미완료입니다. 같은 시각의 표기 차이는 나머지 내용이 같으면 허용합니다. 선택한 원문으로 현재 영향 여부·철회/재활성화·수정 버전을 판단하며, 오래된 원문과 수명 관측은 보존합니다. 출처 기준일을 갱신하거나 미완료 커버리지를 해제하지 않으며, 벌크 아카이브 무결성 규칙과 공유 CVE 행의 조정은 변경하지 않습니다.
...
CLI는 비어 있지 않은 --wanted 파일과 OSWL_GITHUB_ADVISORY_TOKEN 환경변수를 사용하는 온라인 전체 빌드에서 명시적 --sources github-advisory를 지원합니다. 기본 출처는 osv,epss,kev,depsdev로 유지합니다. GitHub 수집은 https://api.github.com만 사용하며 명령행 토큰·base 옵션, offline-sources 및 GitHub 증분은 거절합니다. 완전 수집은 0, 불완전 수집은 확보한 근거와 미확인 키를 작성한 뒤 2를 반환합니다. 설정 오류는 출력 교체 없이 1을 반환합니다. NVD는 아직 미연결입니다. GitHub 데이터는 기준일·수명 정보·기존 출처/라이선스 고지와 함께 별도로 작성하며 github-attributed 배포 프로필이나 다른 출처의 이용 권한을 확대하지 않습니다.
...
온라인 KEV 갱신과 벌크 수집은 중첩 위치를 포함한 중복 JSON 필드와 후행 JSON 값을 거절합니다. 온라인은 HTTP 변환 과정에서 중복 필드가 먼저 소실되지 않도록 응답 바이트를 직접 파싱합니다. 모호한 입력으로 새 카탈로그나 확정 비포함 결과를 만들지 않으며 기존 캐시 근거 정책을 유지합니다. 동일한 합성 입력을 모의 HTTP와 오프라인 소스 파일에 적용해 검사했습니다.

GHSA 벌크 수집 보조 클래스는 정상 0건 조회를 명시적인 컴포넌트 키로 보존하고, 조회 불가·실패는 `unresolvedKeys`로 구분합니다. 부분 조회 예외의 확보된 취약점과 수정 버전 보류 상태를 유지합니다. 미지원·미설정 클라이언트에는 조회를 보내지 않습니다. 온라인 전체 CLI 빌드가 이 보조 클래스를 writer에 연결합니다. 완전한 출처 추적과 GitHub 증분 지원은 별도 작업입니다.

GHSA 스냅샷 변환은 서로 충돌하는 수정 버전 후보도 보존합니다. 충돌이 있으면 수정 버전을 확정하지 않으며, 충돌이 없는 수정 버전은 유지합니다. JSON 저장·복원 후에도 이 구분을 유지하며, 아직 연결되지 않은 CLI 출처를 활성화하는 변경은 아닙니다.

NVD 벌크 helper는 스냅샷 변환 시 CPE 매칭 신뢰도와 공급된 영향 조건 근거를 보존합니다. 후보 하나의 조회 실패가 다른 후보 결과를 버리지 않으며, 부분 응답의 finding을 유지하고 컴포넌트를 `unresolvedKeys`에 표시합니다. 정상 빈 응답은 조회 실패 및 미지원 생태계와 구분합니다. 이 helper는 아직 CLI writer에 연결되지 않았으며, 이번 변경이 `--sources nvd` 활성화나 재배포 권리 확인을 의미하지는 않습니다.

번들이 `basedOnBundleId`를 선언하면 앱은 버전이 있는 증분 메타데이터·기준과 다른 새 번들 ID·MERGE 모드를 요구합니다. 반입 대상 출처의 저장된 ID가 기준과 일치해야 하며, 미확인 상태가 처음 생기는 경우에는 다른 대상 출처의 기준 일치가 필요합니다. 기준 불일치·근거 없음·REPLACE 강제 적용·출처 갱신 후 재적용은 게시 트랜잭션에서 거절합니다. 같은 기준에서 만든 다른 증분을 적용하려면 먼저 전체 번들로 기준을 복원하세요. ID 대조는 일관성 검사이며 출처 인증이나 실제 내용 동일성의 증명이 아닙니다. 과거 전체 번들은 계속 읽을 수 있지만, 증분을 선언한 번들은 버전 메타데이터와 서로 다른 유효한 기준·결과 ID가 필수입니다. CLI도 유효한 ID가 없는 기준으로 증분 생성을 거절하고 기존 출력은 보존합니다. 먼저 새 전체 번들을 생성하세요.

기존 행이 있는 출처에 기준 검증 없는 MERGE를 수행하면 번들 ID를 해제합니다. 이전 행이 남으므로 결과 전체가 새 번들과 같다고 할 수 없기 때문입니다. 전체 REPLACE로 기준을 복원하기 전에는 해당 ID를 기준으로 선언한 후속 증분을 거절합니다. 빈 출처의 최초 반입과 검증된 증분은 결과 ID를 유지합니다. 출처 진위를 인증하거나 구버전이 기록한 계보를 소급 복원하는 기능은 아닙니다.

컴포넌트 범위의 OSV/deps.dev 증분 생성은 이전 번들과 현재 빌드의 `wantedListId`가 같아야 합니다. 값이 다르거나 없으면 기존 출력 파일을 교체하기 전에 거절합니다. 인벤토리가 바뀌었거나 이 메타데이터가 없는 이전 번들은 새 전체 번들을 생성해야 합니다. 식별자는 wanted 파일 원본 바이트의 해시이므로 서식 변경도 새 전체 기준 번들이 필요합니다. KEV/EPSS만 갱신하는 경우에는 wanted 파일 일치를 요구하지 않습니다. 이 방어는 출처별 생태계 범위나 임의 부분 병합의 레코드별 최신성까지 보장하지 않습니다.

KEV/EPSS만 수집하는 빌드는 wanted 파일이 제공되어도 패키지 분석 범위나 미확인 레코드를 생성하지 않습니다. 증분은 선택한 전역 출처만 갱신하고 기존 패키지 미확인 상태와 관련 출처 메타데이터를 유지합니다. 패키지 분석 범위는 OSV 또는 deps.dev 수집을 선택했을 때만 계산합니다.

CLI 번들은 생태계 아카이브 수집·판정이 정상 완료되고 미확인 근거가 없는 구체적인 wanted 패키지에 대해 빈 OSV 결과 행을 저장합니다. 이를 통해 정상 0건 조회가 오프라인 반입 후에도 유지됩니다. 필터로 제외된 생태계, 잘못된 식별자, 미해석 범위는 정상 빈 결과로 바꾸지 않으며 기존 원천 기준일 만료 검사는 유지합니다.

스냅샷 컴포넌트 식별자의 생태계·패키지명·버전에는 내부 키 구분자 `|`를 사용할 수 없습니다. 이런 식별자는 오프라인 조회에서 미확인으로 남고 CLI wanted 입력 및 스냅샷 반입에서 거절하여 서로 다른 좌표가 같은 저장 키로 합쳐지는 것을 막습니다. 정상 scoped npm 이름, Maven 좌표, 배포판 버전의 구분 문자는 기존 키를 유지합니다.

OSV와 GitHub Advisory의 조회 식별자 검증에도 공통 스냅샷 키 규칙을 적용합니다. `|`가 포함된 모호한 좌표는 온라인 빈 응답이나 완료된 스냅샷을 근거로 정상 0건으로 확정하지 않습니다. 정상 좌표의 조회 동작은 유지합니다.

원천 캐시는 미래의 HTTP Last-Modified 시각과 로컬 날짜 파일의 미래 날짜를 기준일 미확인으로 처리합니다. 유효한 HTTP 시각은 UTC 날짜로 변환한 뒤 저장합니다. 미확인 날짜를 다운로드 날짜로 대체하지 않으며, OSV 수집은 계속 확인된 원천 기준일을 요구합니다.

EPSS 벌크 수집은 정규화한 CVE 식별자를 검사한 뒤 점수를 받아들입니다. 잘못된 식별자가 있으면 조용히 건너뛰지 않고 수집을 실패시키며 기존 출력 번들을 보존합니다. 소문자 식별자는 정규화하고 유효한 긴 CVE 일련번호는 유지합니다. 이 검증은 벌크 수집 범위이며 실시간 EPSS 요청 처리 전체를 검증한 것은 아닙니다.

실시간·오프라인 EPSS 요청은 앞뒤 공백과 대소문자를 정규화한 뒤 CVE 형식을 검사하고 중복을 제거합니다. 잘못된 식별자는 HTTP 및 스냅샷 조회에서 제외하며 점수 0으로 대체하지 않습니다. 실제 응답의 점수 0은 유효한 측정값으로 유지합니다.

실시간 EPSS 응답은 중복 JSON 필드와 뒤에 덧붙은 JSON 값을 거절하도록 파싱합니다. 모호한 응답에서는 마지막 필드 값을 선택하지 않고 해당 배치의 점수를 미확인으로 남깁니다. 다른 배치는 독립적으로 계속 조회하며 누락된 점수를 0으로 바꾸지 않습니다.

오프라인 EPSS 번들 반입은 점수 레코드와 삭제 레코드 모두 정규화한 CVE 식별자의 형식을 검증합니다. 잘못된 식별자가 있으면 REPLACE·MERGE 모두 반입을 거절하고 기존 점수와 출처 기준일을 보존합니다. 0점과 유효한 긴 CVE 일련번호는 계속 지원합니다.
...
CLI는 목적지 옆에 임시 번들을 작성하고 앱 반입 단계와 같은 제한으로 검사한 후 최종 파일로 교체합니다. 압축 해제 후 파일/전체 크기·메타데이터 크기·JSONL 행 길이·압축률을 검사합니다. 검증 실패 시 기존 출력과 원본 근거를 보존하고 임시 파일을 정리하며, 크기에 맞추려고 원본을 잘라내지 않습니다. 목적지 파일시스템의 원자적 이동을 사용하므로 이를 지원하지 않으면 실패합니다. 이 전송 형식 검사는 출처 진위·이용 허가·전체 의미 검증을 대신하지 않습니다.

CLI `verify`와 증분 기준 번들 판독도 manifest 검증·비교를 위한 내용 판독 전에 동일한 반입 전송 제한을 적용합니다. 체크섬과 행 수가 일치해도 행 크기 초과·과도한 압축률은 거절합니다. 검증은 입력 파일을 변경하지 않으며 출처 진위나 전체 의미적 유효성을 인증하지 않습니다.

`--distribution-profile github-attributed --wanted <파일>`은 보존된 원본에 지원하는 GitHub Advisory Database 출처가 선언된 OSV 공지만 CLI 출력에 포함합니다. 기본 출처를 `osv`로 선택하며 다른 출처를 명시하면 수집 전에 거절합니다. 출처가 연결되지 않은 공지는 제외하고, 공지가 남은 컴포넌트와 조회 결과가 없는 컴포넌트 모두 전체 탐지 상태는 미확인으로 유지합니다. 출처 기준일·원문 크레딧은 보존합니다. manifest에 `distributionProfile`을 기록하며 증분 기준 전체 번들의 프로파일이 같아야 합니다. 프로파일을 바꾸려면 전체 번들을 다시 생성해야 합니다.

기본값 `unreviewed`는 기존 출처 선택을 유지하며 재배포 승인을 뜻하지 않습니다. 선택 프로파일은 선언된 출처에 따른 출력 제한이지 캐시 진위 인증·포괄적인 권리 판단·기존 반입 데이터의 실행 시 사용 금지가 아닙니다. 다른 자료의 출처별 승인과 앱 수준 배포 통제는 아직 미완료입니다.

앱 반입·CLI 검증·증분 기준 판독은 공통 검증기로 선언된 `github-attributed` 프로파일과 실제 내용을 대조합니다. OSV·unresolved 파일만 허용하고, 각 공지의 ID와 출처가 연결된 원본이 일치해야 하며 전체 번들은 공지가 있는 모든 컴포넌트의 미확인 상태를 유지해야 합니다. 증분을 포함해 미확인 상태를 삭제하는 레코드는 거절합니다. 이는 프로파일 일관성 검사이지 출처 진위 인증이나 법적 승인 증명이 아닙니다. 앱 반입은 게시 트랜잭션 안에서 반입 후 DB도 검사합니다. 해당 프로파일 증분의 모든 OSV 컴포넌트는 기존 또는 함께 반입한 미확인 행을 유지해야 합니다. 미확인 행이 없으면 명시적 REPLACE 요청을 포함해 반입 전체를 거절하고 롤백합니다.

앱은 출처별로 선언된 프로파일을 저장하고 새로 보존하는 세대 메타데이터에도 포함합니다. 기준 ID가 같은 증분을 포함해 MERGE는 저장된 프로파일과 같아야 합니다. 과거 null 프로파일은 이 비교에서 미검토로 취급하며 출처 확인 또는 승인된 값으로 추정하지 않습니다. 전체 REPLACE는 출처의 프로파일을 바꿀 수 있습니다. 이 제어가 내보내기를 필터링하거나 모든 출처 간 탐지 상태 상호작용을 막는 것은 아닙니다.

반입 후 DB에 남는 `github-attributed` OSV 행 전체에도 대응하는 미확인 상태가 필요합니다. 공통 unresolved 파일만 교체해 부분 조회를 완료 상태로 바꿀 수 없으며, 상태가 누락되면 전체 반입을 롤백합니다. OSV 자체를 다른 프로파일의 전체 번들로 교체하는 것은 계속 허용합니다.

스캔 데이터 내보내기는 라이브러리의 집계 조회 상태가 완료여도 저장된 컴포넌트 미확인 상태를 보존합니다. CocoaPods는 패키지 식별자와 OSV 조회용 SwiftURL 식별자 양쪽에 미확인 상태를 기록합니다. 라이브러리 조회 완료 표시만으로 내보내기·재반입 중 불완전한 스냅샷 근거를 지우지 않습니다. 이 동작이 재배포를 승인하거나 원래 번들의 배포 프로파일을 보존하는 것은 아닙니다.

`GET /api/admin/snapshot/export?distributionProfile=github-attributed`는 GitHub 출처가 연결되고 저장된 스캔 판정의 revision/digest와 일치하는 OSV 원본만 내보냅니다. 다른 출처와 여러 출처가 합쳐진 스캔 요약 필드는 제외합니다. 공지가 하나도 남지 않는 경우를 포함해 모든 출력 컴포넌트 식별자는 미확인 상태로 유지합니다. 원문 크레딧과 전달받은 고지는 보존합니다. 알 수 없는 프로파일은 거절하며 기본값 `unreviewed`는 재배포 승인이 아닙니다. 제한 출력도 출처 선언에 따른 필터이지 인증서가 아니며, 누락·변경되었거나 연결 근거가 부족한 원본은 보류합니다. 내보내기 시각으로 원천 최신성을 추정하지 않습니다.
