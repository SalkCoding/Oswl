...
OSV revision 필터·스캔 수집·모니터링은 같은 선택 기준을 사용하며 행 순서와 무관하게 CVE 별칭보다 정확한 공지 ID를 우선합니다. 정확한 공지 행이 중복되거나 정확한 일치 없이 CVE 별칭 행이 여러 개이면 모호한 매칭으로 처리합니다. 원래 관측은 보존하고 조회를 미완료로 표시하며 어느 행도 임의 갱신하지 않습니다. 저장 단계에서 처음 발견된 모호함은 저장 실패로 처리합니다. 모니터링은 OSV 공지 ID가 정확히 일치하는 발견을 우선하고, 없으면 같은 CVE 별칭의 기존 발견에 연결합니다. 두 경로 모두 공지별 revision 검사 후 심각도·출처를 병합하고 누락된 CVSS 쌍·요약·CWE를 채우며 중복 발견 알림을 만들지 않습니다. 저장된 수정 버전 교체는 같은 공지 ID에만 허용합니다. deps.dev 보강 경로도 정확한 OSV 공지를 우선하며 신규 발견을 만들 때를 포함해 같은 공지 ID의 수정 버전만 사용합니다. CVE 별칭만으로 공지별 수정 버전을 확정하지 않습니다. 기존 CVSS 쌍을 함께 유지하고 누락된 심각도를 추측하지 않습니다. 기존 이력·혼합 공급자·불완전 기여 이력은 보수적 심각도 병합을 유지하며 전체 별칭 기여와 철회 조정은 아직 미완료입니다.
