...
스캔 수집 중 DB 저장 실패(낙관적 잠금 충돌 포함)가 발생하면 판정 캡처 전에 중단하고 독립적인 실패 처리로 스캔을 실패 표시합니다. 일부 발견만 저장된 상태를 완료 판정으로 발행하지 않습니다. 동시 쓰기 보호나 공유 라이브러리 전체 갱신의 원자성을 추가한 것은 아니므로 앞서 커밋된 쓰기는 남을 수 있습니다.

V48은 `library_cves.osv_revision_evidence` nullable 열에 공지 ID별로 수용한 OSV revision/digest를 저장합니다. 스캔 수집과 지속 모니터링은 오래되거나 순서를 확인할 수 없거나 같은 revision에서 digest가 다른 관측이 기존 발견을 갱신하지 못하게 합니다. 이미 근거가 있는 공지는 유효한 digest도 필요하며, 더 새로운 유효 revision은 반영할 수 있습니다. 원래 수신 응답은 필터 전 관측 기록에 남습니다. 수집은 거절된 OSV 조회를 미완료로 표시하고 공통 수정 후보를 보류하며 모니터링은 거절된 갱신을 건너뜁니다. 저장 revision JSON이 손상되면 방어를 해제하지 않고 실패합니다. 근거 없는 기존 행은 역으로 채우지 않고 다른 공지 ID는 독립적으로 추적합니다. 출처별 필드 재계산·자동 철회·동시성 잠금을 구현한 것은 아닙니다. 배포 전 V48을 적용하고 rollback 시 열을 유지해야 합니다. 이전 애플리케이션은 이 근거를 관리하지 않으므로 서로 다른 버전의 writer를 혼용하지 마세요. 스캔/오프라인 export에는 아직 이 열이 보존되지 않습니다.
