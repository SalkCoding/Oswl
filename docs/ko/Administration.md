...
OSV 응답이 resolved여도 스냅샷 범위가 미확인이면 심각도 재계산을 허용하지 않습니다. 스캔 수집은 스냅샷 조회 상태를 저장 근거 처리에 전달하고, 모니터링은 원래 컴포넌트 키와 매핑된 OSV 조회 키를 모두 검사합니다. 미확인 키가 있으면 심각도 기여 이력을 불완전으로 기록하고 기존 심각도·CVSS를 낮추지 않습니다.
