...
CLI 수집이 잘못된 updatedAt 문자열을 포함한 GitHub 미완료 조회를 받으면 해당 선택 시각만 알 수 없음으로 저장하고, 매칭된 발견·미확인 컴포넌트 키·수정 후보 보류 상태는 유지합니다. 함께 받은 정상 발견과 뒤따르는 패키지도 계속 수집합니다. 문법상 유효한 미래 시각은 미확인 근거로 번들에 남깁니다. 잘못된 시각 문자열 자체는 타입이 정해진 번들 필드에 보존하지 않으므로 완전한 원문 보존을 의미하지 않습니다.
