...
보안 센터 CSV는 선택한 스캔의 보존된 판정에서 구성 목록, 심각도별 건수, CPE 검토 후보, 패치 가능 여부, 라이선스와 조회 근거를 읽습니다. 이후 공유 라이브러리 갱신으로 이 값들을 바꾸지 않습니다. 보관 처리로 스캔 컴포넌트가 삭제돼도 보존된 행을 내보내며, 컴포넌트가 없으면 범위와 검토·예외 처리 열은 빈칸으로 남깁니다. 컴포넌트가 있으면 현재 검토 상태를 사용합니다. 판정이 없는 과거 스캔은 기존 라이브 데이터 방식을 유지하고, 잘못된 판정은 최신 데이터로 대체하지 않고 실패합니다. 대화형 목록의 전환은 아직 남아 있습니다.
