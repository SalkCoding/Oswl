...
CLI 검증과 증분 기준 판독은 선언된 `github-attributed` 프로파일과 실제 내용을 대조합니다. OSV·unresolved 파일만 허용하고, 각 공지의 ID와 출처가 연결된 원본이 일치해야 하며 전체 번들은 공지가 있는 모든 컴포넌트의 미확인 상태를 유지해야 합니다. 증분을 포함해 미확인 상태를 삭제하는 레코드는 거절합니다. 이는 프로파일 일관성 검사이지 출처 진위 인증이나 법적 승인 증명이 아닙니다.
