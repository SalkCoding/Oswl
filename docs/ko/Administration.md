...
OsWL은 고정된 내장 역할을 제공하지 않습니다 — 관리자가 최초 설정 시 사용 가능한 권한 목록에서 템플릿을 생성합니다.
...
| **사용자명 / 비밀번호** | SMTP 자격증명 (비밀번호는 AES-256-GCM 암호화 저장) |
...
보안 및 라이선스 인사이트 생성에 사용할 LLM 제공업체를 구성합니다.

| 제공업체 | 참고사항 |
|---|---|
| **비활성화** | AI 인사이트 생성 안 함 |
| **OpenAI** | `OPENAI_API_KEY` 필요; 모델 설정 가능 (기본값: `gpt-4o-mini`) |
| **Anthropic** | `ANTHROPIC_API_KEY` 필요; 모델 설정 가능 |
| **로컬** | OpenAI 호환 엔드포인트 (예: Ollama); 기본 URL 및 모델 설정 |

한 번에 하나의 제공업체만 활성화할 수 있습니다.
