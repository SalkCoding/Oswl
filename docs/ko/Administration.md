...
여러 CPE 후보 또는 오프라인 결과 목록이 같은 NVD ID에 서로 다른 근거를 제공하면, 공급자 어댑터는 미채점 검토 후보 하나와 대안 근거 목록(nvdApplicability.conflictingRecords)을 보존하고 조회를 미완료로 표시합니다. JSON 객체 키 순서나 매칭 신뢰도 차이만으로는 충돌로 처리하지 않습니다. 직렬화한 충돌도 이후 조회에서 미확인으로 유지합니다. 도착 순서나 수정 시각으로 충돌을 해소하지 않으며, 기존 공유 행이나 HTTP 페이지 내부의 모든 중복까지 조정하는 것은 아닙니다.
