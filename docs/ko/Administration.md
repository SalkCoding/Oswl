...
수용된 OSV revision 근거에는 이제 공지별 심각도와 CVSS 점수·벡터 쌍도 저장합니다. 생성 시점부터 완전한 이력이 기록된 OSV 단독 행에만 심각도를 재계산합니다. 현재 공지들의 최댓값과 충돌 여부를 사용하고 CVSS 쌍은 같은 심각도의 한 공지에서 가져옵니다(유효 점수 최댓값, 동률이면 공지 ID 순). 명시적으로 심각도가 없는 관측은 미평가 상태이며 발견을 철회·삭제하지 않습니다. 기존 revision JSON·기존 행·혼합 공급자·불완전 조회·revision 없는 기여·이력에 아직 없는 새 CVE 별칭은 하향 정정의 근거로 인정하지 않고 보수적 병합을 유지합니다. 이들의 조정은 잔여 과제입니다. 확장 JSON은 기존 V48 열을 사용하며 이전 애플리케이션이 읽지 못할 수 있으므로 writer를 혼용하거나 호환 DB 백업 없이 애플리케이션만 rollback하지 마십시오. 스냅샷 export는 아직 이 근거를 운반하지 않습니다.
