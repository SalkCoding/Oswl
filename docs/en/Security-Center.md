...
A database integration check feeds the online client synthetic overlapping ranges, packages its selected result in an explicit test ZIP, imports it into H2, and reads it with the offline client. Fresh source metadata preserves the same advisory, severity and 3.0.0 fix. Eight-day-old or missing dates preserve the finding and stored candidate but withhold the offline fix; an absent version key has no completed lookup. The dates are simulated. This verifies the importer/client path, not CLI collection, live GitHub access or PostgreSQL.
