...
OSV bulk collection now shares the lookup identity validator for npm/Cargo, Go, Python, Maven and NuGet versions and the existing package-name checks. Invalid wanted entries are recorded as unresolved even when no advisory mentions them; valid entries continue to be collected. A missing identity that cannot produce an unresolved key fails collection. This does not add version comparators for other ecosystems. CLI output preserves these entries in unresolved.jsonl.

An explicit unresolved OSV result remains unresolved in generated bundles even when another OSV advisory has a known finding or deps.dev has package metadata. Known findings are retained alongside unresolved.jsonl rather than being discarded. This preserves collection uncertainty; it does not establish complete per-provider coverage for every source.

A local CLI → H2 import → offline OSV check covers full-to-delta updates in both directions between explicit uncertainty and completed collection. Known findings and individual fix candidates survive, while unresolved markers are added or removed. Bulk CLI records currently omit original advisory ranges, so a completed lookup still withholds a common fix with NO_RANGE_EVIDENCE; partial lookup uses INCOMPLETE_LOOKUP. This is a recorded limitation, not proof of full online/offline fix parity.

Distribution bulk downloads use the parent OSV buckets Debian, Alpine and Ubuntu, as documented in [OSV ecosystem naming](https://google.github.io/osv.dev/data/#ecosystem-naming). Release identifiers remain part of advisory matching, so one release is not matched to another. On 2026-09-13, HEAD requests to these three all.zip objects returned HTTP 200 and Last-Modified dates of 2026-09-12. Only response headers were inspected; full data completeness and redistribution permissions were not established by this check. Existing release-specific offline source caches need the current parent-bucket filenames.

Release-specific Alpine lookups and bulk collection also apply the existing APK version parser before accepting empty results. Unsupported inputs such as * or latest remain unresolved, while supported revision and prerelease forms remain accepted. Cached finding identities survive, but invalid input cannot produce a fix recommendation. This uses the existing parser coverage and does not add Debian or RPM version comparison.

APK numeric segments retain their text for leading-zero ordering after the initial segment. This follows behavior checked in the [apk-tools version implementation](https://raw.githubusercontent.com/alpinelinux/apk-tools/master/src/version.c) and [blob ordering](https://raw.githubusercontent.com/alpinelinux/apk-tools/master/src/blob.c). Comparator selection accepts both Alpine: and internal ALPINE: forms while package/release matching remains separate. Synthetic tests cover comparison, range membership and bulk fix selection. Upstream GPL-2.0-only source was inspected, not copied or bundled; no native apk execution or complete grammar parity is claimed.

APK ordering also distinguishes a missing numeric segment, suffix number or revision from an explicit zero. For example, 1 precedes 1.0 and 1.0 precedes 1.0-r0. Explicit revision numbers 0 and 00 remain numerically equal. Tests follow the upstream token-order behavior linked above and cover both direct comparison and advisory range/fix selection. Full native APK equivalence remains unverified.

The APK parser does not remove v/V prefixes or surrounding whitespace, and a suffix requires its underscore. Inputs such as 1.0p1 and 1.0rc1 are not silently rewritten to 1.0_p1 and 1.0_rc1. A single trailing letter remains valid and distinct from a suffix. Unsupported provider spellings remain uncertain rather than establishing a package-version alias. This follows the upstream syntax linked above; broader grammar support remains incomplete.

The APK parser accepts a trailing letter followed by an underscore suffix and optional revision, such as 1.0a_p1-r2. Letter comparison precedes suffix comparison; prerelease and patch suffixes retain their ordering. Synthetic tests cover the comparator, OSV range/fix evaluation, bulk conversion, and online/offline lookup identity. Commit-hash versions remain outside this parser support.

Multiple APK suffixes are preserved in order, including absent suffix numbers and the transition to a revision or version end. Pre-release suffixes sort before the corresponding version end; patch suffixes sort after it. Tests cover direct ordering, advisory membership, bulk fix candidates, and online/offline lookup with a multiple-suffix version. Native equivalence and commit-hash support remain unverified.
