...
Patch filtering ranks component/library IDs once, then reads evidence in batches and loads only the selected component entities. In a single local H2 measurement, 5,000 components took 245–566 ms for patch-filter pages after this change versus 3.1–3.6 seconds before it; 50,000 components completed in 2.3–3.3 seconds with exact 25,000-match pagination. These are individual observations, not production latency percentiles. The full JVM, including the in-memory database, approached its 512 MiB heap limit at 50,000 components. Library/evidence retention, concurrent requests and production PostgreSQL scale still require further validation.
