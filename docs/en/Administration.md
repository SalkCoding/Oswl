...
Scan enrichment additionally journals the normalized GitHub lookup result and the deps.dev version response with only the advisory details requested for that component. GitHub lookup flags and partial findings remain intact. For deps.dev, missing requested advisory IDs are listed explicitly; a missing/unresolved version or a missing/stale advisory makes coverage incomplete. A resolved version with no advisory keys is a complete empty lookup. The recorded latest release remains version metadata, not a fixed-version claim. Live GitHub queries now retain supplied updatedAt and validated withdrawnAt values, with withdrawn observations separate from active findings. Both lists reach the journal even after a later page fails or active/withdrawn states conflict; cached findings remain available. Timestamps are observations, not a verified ordering or permission to delete shared findings. A supplied online updatedAt that is malformed, has the wrong type or lies in the future makes the lookup incomplete and withholds fixes while retaining independently matched findings. The same current-time check is used offline. Missing/null revision metadata remains unknown and retains legacy lookup behavior; it is not evidence of revision ordering. Missing or malformed lifecycle metadata, complete historical reconstruction, offline bundle propagation and source reconciliation remain separate work. Existing journal JSON gains optional fields; older strict readers may require a pre-upgrade backup for downgrade. The [official schema](https://docs.github.com/en/graphql/reference/security-advisories#securityadvisory) defines these fields; existing [GitHub Advisory Database attribution terms](https://docs.github.com/en/site-policy/github-terms/github-terms-for-additional-products-and-features#advisory-database) still apply. The journal is not newly included in exports. The GitHub bulk collector helper now maps active and withdrawn observations, including partial lookups, to SnapshotVuln fields githubUpdatedAt/githubWithdrawnAt. Explicit online full CLI builds connect this helper to the writer; see the CLI requirements below. App import preserves them and offline lookup separates withdrawals from active findings. Stale or undated coverage and contradictory states remain incomplete. A future withdrawal date is retained as an unconfirmed finding with no fix, not treated as an effective withdrawal. Legacy bundles without these fields retain their prior behavior. Invalid timestamp types or syntax reject the import. The optional fields are omitted when absent; consumers must understand them before using new bundles. App exports reconstructed from shared CVE rows still do not preserve this lifecycle history.

CLI package bundles now use formatVersion 4 for source-scoped coverage updates in unresolved.jsonl. unresolvedSources adds uncertainty for named sources; resolvedSources clears only those named sources. Full imports apply these updates per key rather than clearing other sources or inventories. Complete refreshes can therefore recover from a prior partial refresh without erasing unrelated uncertainty. Legacy markers with unknown ownership remain unresolved. Current lookup still conservatively treats any remaining marker as incomplete. Upgrade importers before using version 4; older supported formats remain readable. Application exports now preserve stored source ownership in formatVersion 4. Additional uncertainty from an unanalyzed library, and markers whose ownership is unknown, remain explicit legacyUnresolved evidence. Resolving a named source cannot clear that legacy uncertainty. Exports without scoped coverage retain the existing format selection, and the restricted attribution profile remains fully unresolved.

When multiple legacy OSV summary records share an advisory ID but provide different fix versions, offline lookup retains the records and their conflicting candidates but withholds each fix and marks the lookup incomplete. Input order cannot select a winner. Agreement between legacy fixes retains the previous behavior; this is not proof of revision ordering. Original-bearing advisories continue to use their existing revision validation.

For OSV deltas, an unresolved component no longer loses prior findings because its refresh is absent or partial. An absent result retains the previous row; a partial result retains distinct old and new finding objects together, without selecting a winning revision. The unresolved marker remains, so preserved evidence does not establish a complete lookup or confirmed common fix. A completed explicit empty result can still replace prior findings. This preservation rule does not yet cover deps.dev advisory associations or enable GitHub deltas.

An internal writer overload can now write a GitHub-only full payload to github-advisory.jsonl when explicitly given the findings, source date and wanted inventory. It preserves lifecycle fields, successful empty records and unresolved keys, with a manifest and existing GitHub attribution notice. The previous-bundle reader retains these records as well. Explicit online full CLI builds now use this path. GitHub delta writing is rejected until incomplete-refresh evidence preservation is implemented; the existing github-attributed profile still accepts only its attributed OSV originals.

The bundle writer also rejects unsupported selected sources when invoked directly, before staging or replacing output. This prevents an internal caller from bypassing CLI validation and producing a successful bundle that silently omits a requested source. The writer currently supports osv, depsdev, epss and kev; adding a collector requires explicit writer support as well.
...
The CLI supports explicit --sources github-advisory for online full builds with a nonempty --wanted file and OSWL_GITHUB_ADVISORY_TOKEN. Default sources remain osv,epss,kev,depsdev. GitHub collection uses https://api.github.com only; command-line token/base options, offline-sources and GitHub deltas remain rejected. Complete collection returns 0; incomplete collection writes available evidence and unresolved keys, then returns 2. Configuration errors return 1 without replacing output. NVD remains disconnected. GitHub data is written separately with its source date, lifecycle fields and existing attribution/license notice; this does not expand the github-attributed distribution profile or establish rights for other sources.
