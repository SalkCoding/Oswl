...
When a bundle declares `basedOnBundleId`, application import requires versioned delta metadata, a distinct new bundle ID, and MERGE mode. Every affected source must still identify the declared base; unresolved coverage may start empty only when another affected source matches. Mismatched or absent provenance, REPLACE overrides, and replay after the source has advanced are rejected in the publication transaction. Import a full bundle to restore a baseline before applying a different delta derived from it. IDs are consistency metadata, not authentication or proof of exact contents. Legacy bundles without a base declaration retain their previous behavior and do not establish verified lineage.
