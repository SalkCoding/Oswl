...
Each offline OSV query batch reads findings, unresolved keys and source freshness in one independent serializable transaction. A concurrent import cannot give old empty results the new bundle's coverage or date. A subsequent batch can read the committed update. When called inside an existing transaction, this short read needs a separate database connection; leave pool capacity for concurrent readers. This boundary covers one OSV batch, not a whole scan, other sources, later candidate-expiry reads or a durable dataset revision pin.
