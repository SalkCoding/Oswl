...
For an existing finding matched by OSV advisory ID, accepted monitoring results now merge severity/source information and fill missing CVSS pairs, summary and CWE metadata as well as refreshing the fix. Stale revisions remain excluded before these updates. Severity still follows the conservative highest-value/conflict model; downward severity corrections still require source-by-source recomputation. Existing CVSS pairs are retained together, and missing severity is not replaced with a guessed default. CVE-alias-only matching remains a separate reconciliation limitation.
