...
OSV bulk collection now retains the full `osvAdvisory` object when its primary GHSA ID matches the same GitHub Advisory Database source URL on every affected entry. The accepted paths are `github-reviewed` and `unreviewed` JSON records under the official repository. Aliases alone, missing or mixed source declarations, different IDs, and other hosts do not enable original retention. Supplied credits, revision, ranges, and other fields remain intact. `dataNotices.githubAdvisoryDatabase.retainedOriginals` links retained IDs to source URLs under the existing CC-BY-4.0 notice. This recognizes declared attribution, not cache authenticity or permission for other sources. It restores offline common-fix evidence for these records; broader source support and distribution-profile enforcement remain incomplete. Application export still targets scanned libraries, not a full mirror of imported rows.
