...
CLI verification and delta baseline loading check the declared `github-attributed` profile against its contents. Only OSV and unresolved files are allowed; each finding must contain a matching attributed original, and full bundles must retain unresolved coverage for every finding component. Unresolved deletion markers are rejected, including in deltas. These checks validate profile consistency, not source authenticity or a legal approval certificate.
