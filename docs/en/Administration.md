...
When CLI collection receives an incomplete GitHub lookup with malformed updatedAt text, it stores that optional timestamp as unknown and retains the matched findings, unresolved component key and withheld fixes. Valid sibling findings and later packages continue to be collected. Syntactically valid future timestamps remain in the bundle as unconfirmed evidence. The malformed timestamp text itself is not preserved in the typed bundle field; this is not full raw-response preservation.
