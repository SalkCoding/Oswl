...
OSV revision filtering, scan enrichment and monitoring share the same matching rule: the exact advisory ID takes precedence over a CVE alias regardless of row order. Accepted monitoring findings match the exact OSV advisory ID first, then an existing CVE alias. Both paths apply the advisory revision guard, merge severity/source information, and fill missing CVSS pairs, summary and CWE metadata without creating a duplicate discovery alert. Only the same advisory ID may replace the stored fix version. The deps.dev enrichment path also prefers an exact OSV advisory and copies a fix only from that ID, including when creating a finding; a CVE alias alone cannot confirm that advisory-specific fix. Existing CVSS pairs stay together, and missing severity is not guessed. Legacy, mixed-provider or incomplete contribution histories still use conservative severity aggregation; full alias contribution and withdrawal reconciliation remains incomplete.
