...
Preserved scan assessments reject duplicate JSON object fields and any trailing JSON value. Ambiguous stored evidence causes the read to fail; gates, reports, summaries and archive exports do not fall back to current shared findings. The stored record is not rewritten. This checks JSON structure, not provider authenticity or the truth of its contents; embedded source text retains its separate validation rules. Field types are also enforced: fractional format versions or library IDs, numeric enum values, and implicit conversions between strings, numbers and booleans are rejected. Valid assessments written by capture retain their existing format.
...
Lookup completion now rejects future fetch/lookup timestamps and, when source outcomes are recorded, requires a lookup-at timestamp. Preserved assessments store `lookupTimesVerified` at capture and do not revalidate against a later wall clock or current shared cache. A missing flag in an older preserved assessment is incomplete evidence; run a new analysis instead of backfilling the flag. This validates local lookup timestamps, not provider data freshness. Legacy live caches without source outcomes are incomplete, even with a fetch timestamp. They are retried on the next analysis, including with permanent caching; missing historical outcomes are never inferred.
...
Shared patch recommendations also withhold a version while any CPE candidate needs review. Patchability is UNKNOWN, including when a candidate carries a fix string or a cached common-fix record contains the same ID. The raw candidate remains stored; a latest release is not used as a security-fix fallback. Package-provider evidence on the same finding retains the existing common-fix checks. The final PR target verifier keeps its stricter NVD/CPE identity guard.
